...
//   // ================== DASHBOARD (Class + Semester) ==================
//   @Get('dashboard')
//   @Roles('ADVISOR', 'ADMIN')
//   @ApiQuery({ name: 'class_id', required: true, type: String })
//   @ApiQuery({ name: 'semester_id', required: false, type: String })
//   @ApiQuery({ name: 'q', required: false, type: String, description: 'Tìm theo mã SV / họ tên' })
...
//   ) {
//     return this.advisorService.listWarnings(req.user.userId, classId, semesterId, status, page, limit);
//   }
...
export class AdvisorController {
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Tìm theo mã SV / họ tên',
  })
  @ApiQuery({ name: 'page', required: false, type: Number, example: 1 })
  @ApiQuery({ name: 'limit', required: false, type: Number, example: 10 })
  async classDashboard(@Req() req: any, @Query() dto: GetAdvisorDashboardDto) {
  ...
  async studentDetail(
    ...
    return this.advisorService.getStudentDetail(
      req.user.userId,
      studentId,
      dto,
    );
  ...
  async studentTimeline(
    ...
    return this.advisorService.getStudentTimeline(
      req.user.userId,
      studentId,
      semesterId,
    );
  ...
  async classAnalytics(
    ...
    return this.advisorService.getClassAnalytics(
      req.user.userId,
      classId,
      semesterId,
    );
  ...
  async classCourseStats(
    ...
    return this.advisorService.getClassCourseStats(
      req.user.userId,
      classId,
      semesterId,
    );
